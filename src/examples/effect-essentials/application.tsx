    ...
    <div className="space-y-6">
      <div className="rounded-md bg-red-50 p-4 dark:bg-red-900/20">
        ...
        <p className="text-sm text-red-700 dark:text-red-300">
          The effect only runs on mount. When you click different users, the effect doesn&apos;t
          re-run because <code className="rounded bg-red-100 px-1 dark:bg-red-800">userId</code> is
          not in the dependency array. The UI shows the same user no matter what you click.
        </p>
      </div>
    ...
    <div className="space-y-6">
      <div className="rounded-md bg-yellow-50 p-4 dark:bg-yellow-900/20">
        ...
        <p className="text-sm text-yellow-700 dark:text-yellow-300">
          Dependencies are correct now, but there&apos;s no cleanup. If you rapidly click different
          users, requests return in unpredictable order due to random delays. You might see User
          2&apos;s data even though you selected User 3. Click rapidly between users to see the race
          condition!
        </p>
      </div>
      ...
      <div className="space-y-6">
        <div className="rounded-md bg-slate-100 p-8 text-center dark:bg-slate-800">
          <p className="mb-4 text-slate-600 dark:text-slate-400">
            Component unmounted. Check your browser console for warnings about setting state on
    ...
    <div className="space-y-6">
      <div className="rounded-md bg-orange-50 p-4 dark:bg-orange-900/20">
        ...
        <p className="mb-2 text-sm text-orange-700 dark:text-orange-300">
          When you select a user and then click &quot;Unmount Component&quot;, the fetch completes
          and tries to call{' '}
          <code className="rounded bg-orange-100 px-1 dark:bg-orange-800">setUser</code> and{' '}
          <code className="rounded bg-orange-100 px-1 dark:bg-orange-800">setLoading</code> on an
          unmounted component. Check your browser console for React warnings.
        </p>
        <button
      ...
      try {
        ...
        if (!cancelled) {
          ...
          addMessage(
            userId,
            'received',
            `Received user ${userId}: ${userData.name}`,
            userData.name,
          );
        } else {
    ...
    <div className="space-y-6">
      <div className="rounded-md bg-green-50 p-4 dark:bg-green-900/20">
        ...
        <p className="text-sm text-green-700 dark:text-green-300">
          Now we have proper cleanup with a cancellation flag. When you rapidly click between users,
          the cleanup function sets{' '}
          <code className="rounded bg-green-100 px-1 dark:bg-green-800">cancelled = true</code>,
          preventing stale data from being set. The console shows cancelled requests. Try rapidly
          clicking between users - no more race conditions!
        </p>
      </div>
    ...
    <Container className="my-8 space-y-8">
      <section>
        ...
        <p className="text-slate-600 dark:text-slate-400">
          Understanding common useEffect mistakes and how to fix them. Each tab demonstrates a
          different problem with progressive solutions.
        </p>
      </section>
          ...
          <div className="flex space-x-4">
            {versions.map((version) => (
              <button
                ...
                className={`border-b-2 px-4 pt-2 pb-3 text-sm font-medium transition-colors ${
